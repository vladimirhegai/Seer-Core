...
function agentsBlock(): string {
  return [
    ...
    '   ambiguous name the response carries a `nameAmbiguity` hint naming the',
    '   chosen definition and alternatives — re-call with `file` if it picked wrong.',
    '',
    ...
    '  a resolved count far below reality is reported under `ambiguity` — narrow it',
    '  with `groupByFile`, `filterReceiverType`, or `includeNameMatches` (or import',
    '  a SCIP index for an exact count).',
